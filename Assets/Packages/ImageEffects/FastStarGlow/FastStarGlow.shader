Shader "ImageEffect/FastStarGlow"
{
    Properties
    {
        [HideInInspector]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                // NOTE:
                // Add symmetric blurred color to make this effect fast.
                ...
                for (int j = 0; j < 4; j++)
                {
                    color += (tex2D(_MainTex, uv1) + tex2D(_MainTex, uv2)) * pow(ATTENUATION, input.power * j);
                    uv1 += input.offset;
                    uv2 -= input.offset;
                }
