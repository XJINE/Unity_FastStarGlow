...
public class FastStarGlow : ImageEffectBase
{
    #region Enum
    ...
    public FastStarGlow.CompositeType compositeType = FastStarGlow.CompositeType._COMPOSITE_TYPE_ADDITIVE;

    [Range(0,  10)] public float threshold     = 1.0f;
    [Range(0,   1)] public float intensity     = 0.1f;
    [Range(1,  20)] public int   divide        = 3;
    [Range(1,   5)] public int   iteration     = 5;
    [Range(0,   1)] public float attenuation   = 1.0f;
    [Range(0, 360)] public float angleOfStreak = 0.0f;
    ...
    protected override void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        float angle = 360f / (this.numOfStreak * (this.numOfStreak % 2 == 0 ? 2 : 1));

        for (int x = 1; x <= this.numOfStreak; x++)
        {
            Vector2 offset = (Quaternion.AngleAxis(angle * x + this.angleOfStreak, Vector3.forward) * Vector2.down);
        ...
        base.material.EnableKeyword(FastStarGlow.CompositeTypes[this.compositeType]);
        ...
        base.material.DisableKeyword(FastStarGlow.CompositeTypes[this.compositeType]);
